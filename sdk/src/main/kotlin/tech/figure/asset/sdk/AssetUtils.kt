...
class AssetUtils (
    ...
    val osClient: OsClient = OsClient(OsClientURI(), config.osConfig.timeoutMs)
    ...
    fun encryptAndStore(
        ...
        val future = osClient.put(asset, encryptPublicKey, signerPublicKey, signature)
        val res: Objects.ObjectResponse = future.get(config.osConfig.timeoutMs, TimeUnit.MILLISECONDS)
        return res.hash.toByteArray()
    }
     */

    // Encrypt and store an asset using a random keypair for the signer
    fun encryptAndStore(
        asset: Message,
        encryptPublicKey: PublicKey,
    ): ByteArray {
        val future = osClient.put(asset, encryptPublicKey, Pen(ProvenanceKeyGenerator.generateKeyPair(encryptPublicKey)))
        val res: Objects.ObjectResponse = future.get(config.osConfig.timeoutMs, TimeUnit.MILLISECONDS)
        return res.hash.toByteArray()
    }
    ...
    private fun OsClientURI(): URI {
        val scheme = if(config.osConfig.secure) "grpcs" else "grpc"
        return URI.create("${scheme}://${config.osConfig.host}:${config.osConfig.port}")
    }
